 ...
 * Usage:
 *   npx s8vr           — install
 *   npx s8vr update    — pull latest, rebuild, restart
 *   npx s8vr config    — reconfigure environment variables
 *   npx s8vr restart   — restart s8vr services
 */
...
if (process.platform === 'win32') {
  ...
  console.log('');
  console.log('  Once inside WSL, run: npx s8vr');
  console.log('');
  process.exit(1);
}
...
async function cmdUpdate() {
  ...
  if (hasPm2()) {
    ...
    if (fs.existsSync(ecosystemPath)) {
      ...
      warn('No ecosystem.config.js found — run: s8vr restart');
    }
  } else {
...
async function cmdInstall() {
  ...
  const frontendEnv = [
    `# s8vr Frontend Configuration`,
    `# Generated by s8vr on ${timestamp}`,
    ``,
    `VITE_API_URL=${backendUrl}`,
